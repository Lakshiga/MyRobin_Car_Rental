    ...
    <Box className="space-y-16 md:space-y-24">
      ...
      <Box className="text-center space-y-6">
        <Box className="space-y-4">
          <Chip
            ...
            className="!bg-blue-100 !text-blue-700 !font-semibold !text-sm !px-3 !py-1"
        ...
        <Box className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          <Button
            ...
            className="!px-8 !py-4 !border-2 !border-blue-500 !text-blue-600 hover:!bg-blue-50 !rounded-xl"
          >
            Manage Booking
          </Button>
        </Box>
        ...
        <Grid container spacing={3}>
          {brands.map((brand, index) => (
            <Grid item xs={6} sm={3} key={index}>
              <Card className="!rounded-2xl hover:!shadow-xl !transition-all !cursor-pointer !bg-gradient-to-br !from-slate-900 !to-slate-800">
                <CardContent className="!p-4 !text-center">
                  <Avatar className="!h-16 !w-16 !mx-auto !mb-2 !bg-white !text-slate-900 !font-bold !text-xl">
                    {brand.logo}
                  </Avatar>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={4}>
          <Card className="!rounded-2xl hover:!shadow-xl !transition-all !h-full">
        ...
        <Grid container spacing={3}>
          {popularCars.map((car) => (
            <Grid item xs={12} sm={6} lg={4} key={car.id}>
              <Card
                className="!rounded-2xl hover:!shadow-xl !transition-all !transform hover:!-translate-y-1 !cursor-pointer !h-full"
                onClick={() => handleViewDetails(car.id)}
              >
                <Box className="relative">
                  <Box className="flex h-48 items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 rounded-t-2xl text-7xl">
                    {car.image}
                  </Box>
                  <IconButton
                    size="small"
